...
> Living document. Update whenever behaviour changes. Last updated: 2026-09-03.
>
...
- Restricted browser pages (`chrome://`, `about:`, the Web Store, and similar) cannot be scripted; the click does nothing there.

### Firefox support

- The Firefox package is a Manifest V3 WebExtension in `Firefox/`.
- Supported floor: Firefox 128 or newer, including Firefox ESR releases based on 128 or newer.
- Firefox uses its `browser.*` APIs and a non-persistent background script; the page-scoped interval remains in the injected script.
- Install the reproducible unsigned development package generated by `scripts/package-firefox.sh` (output: `publish/autoscroll-firefox.xpi`). Firefox Add-ons signing and submission are publishing work, not performed by this repository.
- Manual cross-browser evidence is recorded in [`cross-browser-test-matrix.md`](./cross-browser-test-matrix.md).
